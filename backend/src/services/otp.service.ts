...
export function verifyOtp(email: string, code: string): { valid: boolean; error?: string; status?: number } {
  ...
  if (entry.code !== code) {
    ...
    return { valid: false, error: `Invalid OTP. ${remaining} attempt${remaining === 1 ? "" : "s"} remaining.`, status: 401 };
  }
