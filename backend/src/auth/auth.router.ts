...
router.post('/signup/send-otp', async (req: Request, res: Response) => {
  ...
  if (!rateCheck.allowed) {
    res
      .status(429)
      .json({ error: `Too many requests. Try again in ${rateCheck.retryAfterSecs}s.` });
    return;
  }
...
router.post('/signup/send-recovery-link', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!existing) {
      ...
      res
        .status(200)
        .json({ message: 'If this email is registered, a recovery link has been sent.' });
      return;
    }
  } catch (err) {
